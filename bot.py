...
CAREER_HINTS_DDO = {
    "Ч-П": "🌿 Человек-Природа\n\nВам подходят профессии, связанные с природой: биолог, агроном, ветеринар, эколог, геолог, лесник, флорист.\n\nРекомендация: изучайте биологию, химию, экологию.",
    "Ч-Т": "🔧 Человек-Техника\n\nВам подходят технические профессии: инженер, программист, механик, электрик, строитель, технолог.\n\nРекомендация: развивайтесь в математике, физике, информатике.",
    "Ч-Ч": "👥 Человек-Человек\n\nВам подходят социальные профессии: врач, учитель, психолог, менеджер, юрист, журналист.\n\nРекомендация: развивайте коммуникативные навыки, изучайте психологию.",
    "Ч-З": "📊 Человек-Знаковая система\n\nВам подходят профессии, связанные с обработкой информации: бухгалтер, экономист, программист, переводчик, аналитик.\n\nРекомендация: изучайте математику, языки, программирование.",
    "Ч-Х": "🎨 Человек-Художественный образ\n\nВам подходят творческие профессии: дизайнер, художник, музыкант, актёр, писатель, архитектор.\n\nРекомендация: развивайте творческие способности, изучайте искусство.",
}
...
CAREER_HINTS_OPG = {
    "ПОЗ": "📚 Познавательная готовность — собирайте факты: профессии, требования, вузы, рынок труда; консультируйтесь с педагогом.",
    "ЭМО": "💚 Эмоциональная готовность — нормализуйте тревогу, опирайтесь на ценности; при сильном стрессе обратитесь к школьному психологу.",
    "ДЕЯ": "🎯 Деятельностная готовность — малые шаги: профориентация, проекты, практика; фиксируйте цели на неделю/месяц.",
    "КОМ": "🤝 Коммуникативная готовность — обсуждайте планы с близкими и специалистами; учитесь формулировать запросы о помощи.",
}

# --- Таблица ОПТ: вопросы из JSON (модификация Резапкиной) ---
QUESTIONS_JOVASHI = _load_questions(JOVASHI_PATH)

# --- Йовайши: отдельная формулировка вопросов, та же логика подсчёта сфер ---
...
CAREER_HINTS_KETTELL = {
    "ПР": "📐 Прагматичность — сильные стороны в анализе, планировании, точных задачах. Подумайте об инженерии, IT, экономике, управлении качеством.",
...
WELCOME_TEXT = (
    ...
    "Доступные тесты:\n"
    "• ДДО (дифференциально-диагностический опросник Климова) — 30 пар занятий, выберите одно; "
    ...
    "Можно начать тест кнопкой внизу или командой в чат: ддо, опг, таблица (или опт), йовайши (или йоваши), кеттелл, равен, "
    "эн-60, эн-57. Слово «меню» или «привет» снова покажет это сообщение.\n\n"
    "Для админа: скачать все ответы в Excel — напишите одно слово «выгрузка» или «/stats». "
    "Нужно указать id ВК администратора в переменной STATS_ADMIN_IDS на сервере бота (Railway)."
)
...
def get_progress(user_id: int):
    with db_connect() as conn:
        ...
        if tid == TEST_DDO:
            ddo_keys = set(PROFESSION_TYPES)
            sk = set(scores.keys())
            nq = len(QUESTIONS_DDO)
            dirty = False
            if sk != ddo_keys or "Ч-С" in scores:
                merged = empty_scores(TEST_DDO)
                for k in ddo_keys:
                    merged[k] = int(scores.get(k, 0))
                if "Ч-С" in scores:
                    merged["Ч-Т"] += int(scores["Ч-С"])
                scores = merged
                dirty = True
...
def handle_stats_command(vk, user_id: int) -> bool:
    if not is_stats_admin(vk, user_id):
        send_message(
            ...
            "Сохраните и Redeploy. Потом напишите боту одно слово: выгрузка",
        )
        return True
    try:
...
def finish_test(vk, user_id: int, test_id: str, scores: dict):
    ...
    if tid == TEST_DDO:
        title = "📊 Ваш результат по тесту ДДО (топ-3):"
        lines = [title]
        ...
        lines = [f"📊 Ваш результат по «{LABEL_KETTELL}» (топ-3 черты):"]
        for i, (key, points) in enumerate(top3, 1):
        ...
        lines = [
            ...
            "Используйте как тренировку внимательности и логики.",
        ]
        if pct >= 75:
            lines.append("\nСильный результат — продолжайте решать подобные задачи и разбирать ошибки.")
        elif pct >= 50:
            lines.append("\nСредний уровень — полезно тренировать ряды, условия и аккуратность в подсчётах.")
        else:
            lines.append("\nЕсть куда расти — разбирайте каждую задачу и ищите закономерность.")
